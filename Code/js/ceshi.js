$(document).ready(function ()
{
    ...
    selectTimu(qid);
    $("span#jishimin").text('60');
    $("span#jishisec").text('00');
...
function changeTime()
{
    var min = $("span#jishimin").text();
    var sec = $("span#jishisec").text();
    min = parseInt(min);
    sec = parseInt(sec);
    if (min >= 0)
    {
        ...
        $("span#jishimin").text(min);
        $("span#jishisec").text(sec);
        setTimeout(changeTime, 1000);
    }
}
/*ajax操作*/
...
function xiayiti()
{
    ...
    {
        if (window.confirm('已经是第一题啦！确定交卷？'))
        {
            jiaojuan();
        }
    }
...
function jiaojuan()
{
    $("#mainloading").css("display","");
    document.getElementById("kaoshi").submit();
}
